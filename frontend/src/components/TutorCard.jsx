...
const TutorCard = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (selectedSchedule && student) {
      ...
      const updatedStudentAppointments = [
        ...student.appointments,
        newAppointmentForStudent,
      ];
      const updatedTutorAppointments = [
        ...tutor.appointments,
        newAppointmentForTutor,
      ];
      ...
      const updatedStudent = {
        ...studentWithoutId,
        appointments: updatedStudentAppointments,
      };
      ...
      try {
        const studentResponse = await fetch(
          `http://localhost:3000/api/students/update/${student._id}`,
          {
            method: "PUT",
            headers: {
        ...
        const tutorResponse = await fetch(
          `http://localhost:3000/api/tutors/update/${tutorId}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
        ...
        if (studentResponse.ok && tutorResponse.ok) {
          ...
          console.error(
            "Failed to update student appointments:",
            studentErrorData
          );
  ...
  return (
    <div className="mx-auto flex h-auto w-full max-w-screen-lg items-center justify-center py-10">
